...
class Alerts{
    ...
    static func failedPayment(){
        _ = SweetAlert().showAlert("ORDER FAILED", subTitle: "Check your internet connection and try again", style: .error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    
    static func cardDeclined(){
         _ = SweetAlert().showAlert("DECLINED", subTitle: "Something went wrong processing your payment", style: .error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    
    static func emailSaveFailed(){
        _ = SweetAlert().showAlert("SAVE FAILED", subTitle: "Check your internet connection and try again", style: .error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    
    static func saveNotSuccesful(isCard: Bool, internetError: Bool){
        let string = isCard ? "Card" : "Address"
        _ = SweetAlert().showAlert("SAVE FAILED", subTitle: "\(string) could not be saved. Check your internet connection and try again.", style: AlertStyle.error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    
    static func duplicate(isCard: Bool){
        let string = isCard ? "card" : "address"
        _ = SweetAlert().showAlert("DUPLICATE", subTitle: "You already have this \(string) on file", style: .warning, buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    
    static func failedDeleteAlert(_ isCard: Bool){
        let string = isCard ? "Card" : "Address"
        _ = SweetAlert().showAlert("DELETE FAILED", subTitle: "\(string) could not be deleted. Check your internet and try again later", style: .error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    ...
    static func catchall(_ action: @escaping (Bool) -> Void){
        _ = SweetAlert().showAlert("ERROR", subTitle: "Something went wrong on our end. Please log in again.", style: .error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue, action: action)
    }
    
    static func unauthenticated(_ action: @escaping (Bool) -> Void){
        _ = SweetAlert().showAlert("SESSION EXPIRED", subTitle: "Your session has expired. Please log in again.", style: .warning, buttonTitle: "OKAY", buttonColor: Constants.darkBlue, action: action)
    }
    ...
    static func accountExists(_ completion: @escaping (Bool)->Void){
    ...
    static func serverError(){
        _ = SweetAlert().showAlert("SERVER ERROR", subTitle: "Please try again later", style: .error,  buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    
    static func noAccount(){
        _ = SweetAlert().showAlert("NO ACCOUNT", subTitle: "No account with this phone number was found", style: .error,  buttonTitle: "OKAY", buttonColor: Constants.darkBlue)
    }
    static func holdUp(_ string: String){
        _ = SweetAlert().showAlert("HOLD UP", subTitle: "We're still processing your \(string), give us one second", style: .none, buttonTitle: "OKAY", buttonColor: Constants.darkBlue, action: nil)
    }
    
    static func passwordReset(){
        _ = SweetAlert().showAlert("PASSWORD RESET", subTitle: "Your password was sucessfully reset", style: .none, buttonTitle: "OKAY", buttonColor: Constants.darkBlue, action: nil)
    }
    
    static func overload(_ sc: SliceController){
        _ = SweetAlert().showAlert("OVERLOAD", subTitle: "We have an 8 slice maximum for now, sorry!", style: .warning, buttonTitle: "OKAY", buttonColor: Constants.darkBlue){
            _ in sc.orderProgressBar?.timer.resume()
        }
    }
    
    static func noAddresses(_ na: NewAddressController){
        _ = SweetAlert().showAlert("NETWORK ERROR", subTitle: "Failed to fetch list of active dorms. Please check your network connection", style: .error, buttonTitle: "OKAY", buttonColor: Constants.darkBlue){ _ in

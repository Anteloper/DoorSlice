...
open class SweetAlert: UIViewController {
    ...
    var kContentWidth: CGFloat = 315.0
    ...
    fileprivate func resizeAndRelayout() {
        ...
        let string = button.title(for: UIControlState())!
        let at = Constants.getTitleAttributedString(button.title(for: UIControlState())!, size: 18, kern: 5.0)
        at.addAttribute(NSForegroundColorAttributeName, value: color.cgColor, range: (at.string as NSString).range(of: string))
        button.setAttributedTitle(at, for: UIControlState())
        ...
        if buttons.count == 1{
            ...
            let string2 = (otherButton.title(for: UIControlState()))!
            let at2 = Constants.getTitleAttributedString(string2, size: 18, kern: 5.0)
            at2.addAttribute(NSForegroundColorAttributeName, value: color.cgColor, range: (at2.string as NSString).range(of: string2))
            otherButton.setAttributedTitle(at2, for: UIControlState())
            otherButton.addTarget(self, action: #selector(pressed(_:)), for: .touchUpInside)
        ...
        contentView.sendSubview(toBack: button)
        y = button.frame.maxY
...
class SuccessAnimatedView: AnimatableView {
    ...
    func setupLayers() {
        ...
        circleLayer.fillColor = UIColor.clear.cgColor;
        circleLayer.strokeColor = Constants.seaFoam.cgColor;
        circleLayer.lineCap = kCALineCapRound
        circleLayer.lineWidth = 4;
        circleLayer.actions = [
            "strokeStart": NSNull(),
